( function($, woosa){
   ...
   var moduleOrderColumnStatus = {
      ...
      /**
       * Displays the status on order table column.
       */
      render_table_column: function(){

         //at initial page load
         $('[data-'+Prefix+'-order-table-column-status]').each(function(){
            moduleOrderColumnStatus.render_table_column_status($(this));
         });
         ...
         $(document).on('click', '.row-actions .editinline', function(e){
            ...
            var _this   = $(this),
               ...
               if($('#'+row_id).is(':visible')){
                  $('#'+row_id).find('[data-'+Prefix+'-order-table-column-status]').each(function(){
                     moduleOrderColumnStatus.render_table_column_status($(this));
                  });
      ...
      render_table_column_status: function(elem){
         ...
         var args = JSON.parse(elem.attr('data-'+Prefix+'-order-table-column-status'));

         $.ajax({
            url: Ajax.url,
            method: "POST",
            data: {
               action: Prefix+'_render_order_table_column_status',
               security: Ajax.nonce,
               args: args,
            },
