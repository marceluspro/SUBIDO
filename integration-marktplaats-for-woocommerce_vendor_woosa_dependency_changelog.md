## 1.2.2 - 2025-09-05

### Changed

* Remove translation to avoid the warning of use it too earlier

## 1.2.1 - 2024-12-13

### Fixed

* Getting plugin data translated (by using `get_plugin_data()`) triggers the warning: *Function `_load_textdomain_just_in_time` was called incorrectly[...]*

## 1.2.0 - 2024-02-20

### Changed

* The module does not require WooCommerce plugin anymore

## 1.1.1 - 2023-04-27

### Changed

* The method `get_active_wp_plugins()` is not public to be access from outside

## 1.1.0 - 2022-08-04

* [CHANGE] - Increase the minimum version for PHP from `v7.1` to `v7.4` and for WooCommerce from `v3.5` to `v5.0`